...
import tgt
from praatio import textgrid as Textgrid
import librosa
import numpy as np
...
class Preprocessor:
    ...
    def build_from_path(self):
        ...
        os.makedirs((os.path.join(self.out_dir, "alignment")), exist_ok=True)
            ...
            for wav_name in os.listdir(os.path.join(self.in_dir, speaker)):
                ...
                if self.dataset=="Baker":
                    tg_path = os.path.join(
                        self.out_dir, "TextGrid", speaker, "{}.interval".format(basename)
                    )
                else:
                    tg_path = os.path.join(
        ...
        pitch_min, pitch_max = self.normalize(
            os.path.join(self.out_dir, "f0"), pitch_mean, pitch_std
        )
    ...
    def process_utterance(self, speaker, basename):
        ...
        if self.dataset=="Baker":
            ...
            textgrid=Textgrid.openTextgrid(
                tg_path, includeEmptyIntervals=True)
            phone, duration, start, end = self.get_alignment(
                textgrid.tierDict[textgrid.tierNameList[0]].entryList)
        else:
            tg_path = os.path.join(
                self.out_dir, "TextGrid", speaker, "{}.TextGrid".format(basename)
            )
